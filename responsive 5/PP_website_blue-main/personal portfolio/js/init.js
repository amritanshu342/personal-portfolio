...
function iknow_tm_menu_width() {
    "use strict"; var ww = jQuery(window).width(); var btn = jQuery('.iknow_tm_hero .main_menu ul li a'); btn.on('mouseenter', function () {
        var element = jQuery(this); var li = element.closest('li'); if (li.hasClass('active') || li.hasClass('entered')) { return false; }
        li.closest('ul').children().removeClass('entered'); li.addClass('entered'); var elWidth = li.outerWidth(); var textWidth = li.find('span').outerWidth(); li.css({ width: elWidth + textWidth + 'px' });
    }).on('mouseleave', function () {
        var element = jQuery(this); var li = element.closest('li'); var allLi = li.closest('ul').children(); var fixedLi = li.closest('ul').children('.fixed'); var activeLi = li.closest('ul').children('.active'); allLi.removeClass('entered').css({ width: '135px' }); if (ww <= 1400) { allLi.removeClass('entered').css({ width: '95px' }); }
        if (activeLi.length) {
            var elWidth = 135; if (ww <= 1400) { elWidth = 95; }
            var textWidth = activeLi.find('span').outerWidth(); activeLi.removeClass('fixed').addClass('active').css({ width: elWidth + textWidth + 'px' });
        }
    }); btn.on('click', function () {
        var element = jQuery(this); var li = element.closest('li'); var href = element.attr('href'); if (li.hasClass('active') && !li.hasClass('entered')) { return false; }
        ...
        iknow_tm_owl_carousel(); return false;
    }); jQuery('.iknow_tm_hero .main_menu ul li.active').each(function () { var element = jQuery(this); var elWidth = element.outerWidth(); var textWidth = element.find('span').outerWidth(); var href = element.find('a').attr('href'); element.css({ width: elWidth + textWidth + 'px' }); $(href).addClass('active'); });
}
...
function iknow_tm_contact_form() {
    "use strict"; jQuery(".contact_form #send_message").off().on('click', function () {
...
new WOW().init(); function iknow_tm_owl_carousel() {
    "use strict"; var carousel = jQuery('.iknow_tm_testimonials .owl-carousel'); var carousel2 = jQuery('.iknow_tm_fixed_content .iknow_tm_testimonials .owl-carousel'); var rtlMode = false; if (jQuery('body').hasClass('rtl')) { rtlMode = 'true'; }
    carousel2.owlCarousel({ items: 1, }); carousel.owlCarousel({ loop: true, items: 2, lazyLoad: false, margin: 25, autoplay: true, autoplayTimeout: 7000, rtl: rtlMode, dots: true, nav: false, navSpeed: false, responsive: { 0: { items: 1 }, 768: { items: 2 } } }); iknow_tm_imgtosvg();
}
function iknow_tm_circular_progress() {
    "use strict"; var ww = jQuery(window).width(); var circVal; if (ww > 1400) { circVal = 213; }
    else if (ww >= 768) { circVal = 170; }
    else { circVal = 120; }
    jQuery('.circular_progress_bar .myCircle').each(function () { var element = jQuery(this); element.append('<h3 class="number"></h3>'); var value = element.data('value'); element.circleProgress({ size: circVal, value: 0, animation: { duration: 1400 }, thickness: 5, fill: " #6563ff", emptyFill: 'rgba(0,0,0,0)', startAngle: -Math.PI / 2 }).on('circle-animation-progress', function (event, progress, stepValue) { element.find('h3').text(parseInt(stepValue.toFixed(2) * 100) + '%'); }); element.circleProgress('value', 1.0); setTimeout(function () { element.circleProgress('value', value); }, 1400); });
}
function by_frenify(wrapper, speed) {
    "use strict"; var ww = jQuery(window).width(); var circVal; if (ww > 1400) { circVal = 213; }
    else if (ww >= 768) { circVal = 170; }
    else { circVal = 120; }
    var element = wrapper.find('.myCircle'); if (element.length > 0) {
        element.each(function () {
            var e = jQuery(this); var value = e.data('value'); var speed2 = 1400; if (!e.find('.number').length) { e.append('<h3 class="number"></h3>'); }
            e.circleProgress({ size: circVal, value: 0, animation: { duration: speed2 }, thickness: 5, fill: " #6563ff", emptyFill: 'rgba(0,0,0,0)', startAngle: -Math.PI / 2 }).on('circle-animation-progress', function (event, progress, stepValue) { e.find('h3').text(parseInt(stepValue.toFixed(2) * 100) + '%'); }); setTimeout(function () { e.circleProgress('value', 1.0); }, speed); setTimeout(function () { e.circleProgress('value', value); }, speed2 + speed);
        });
    }
}
function iknow_tm_extra_menu() {
    "use strict"; var item = jQuery('.iknow_tm_extra_menu .unorderest_li'); var speed = 1000; item.each(function () { var element = jQuery(this); var content = jQuery(element.find('.iknow_tm_full_link').attr('href')).html(); element.append('<div class="details_wrapper"></div>').find('.details_wrapper').html(content); }); var button = jQuery('.iknow_tm_extra_menu .iknow_tm_full_link'); button.on('click', function () {
        var element = jQuery(this); var li = element.closest('li'); var details = li.find('.details_wrapper'); var detailsH = details.outerHeight() * 0.7; var min = 500, max = 1000; speed = detailsH < min ? min : detailsH; speed = speed > max ? max : speed; speed = parseInt(speed); if (li.hasClass('opened')) { li.removeClass('opened').find('.details_wrapper').slideUp(speed); return false; } else { li.siblings().removeClass('opened'); li.addClass('opened'); }
...
function iknow_tm_page_transition() {
    "use strict"; var section = jQuery('.iknow_tm_animate_section'); var allLi = jQuery('.transition_link li'); var button = jQuery('.transition_link a'); var wrapper = jQuery('.iknow_tm_all_wrap'); var enter = wrapper.data('enter'); var exit = wrapper.data('exit'); button.on('click', function () {
        var element = jQuery(this); var href = element.attr('href'); if (element.parent().hasClass('iknow_tm_button')) { jQuery('.menu.transition_link a[href="' + href + '"]').trigger('click'); hashtag2(); return false; }
...
function iknow_tm_trigger_opener() {
    "use strict"; var audio1 = jQuery('#audio1'); var audio2 = jQuery('#audio2'); var hamburger = jQuery('.iknow_tm_extra_demo .iknow_tm_topbar .trigger .hamburger'); var menu = jQuery('.iknow_tm_topbar .wrapper .menu'); hamburger.on('click', function () {
        var element = jQuery(this); if (element.hasClass('is-active')) { element.removeClass('is-active'); audio1[0].play(); menu.removeClass('opened'); } else { element.addClass('is-active'); audio2[0].play(); menu.addClass('opened'); }
        return false;
    });
}
function iknow_tm_trigger_menu() {
    "use strict"; var hamburger = jQuery('.iknow_tm_mobile_menu .trigger .hamburger'); var mobileMenu = jQuery('.iknow_tm_mobile_menu .dropdown'); var mobileMenuList = jQuery('.iknow_tm_mobile_menu .dropdown .dropdown_inner ul li a'); hamburger.on('click', function () {
        ...
        return false;
    }); mobileMenuList.on('click', function () { jQuery('.iknow_tm_mobile_menu .trigger .hamburger').removeClass('is-active'); mobileMenu.slideUp(); return false; });
}
function iknow_tm_menu_width_action(element, action) {
    "use strict"; var ww = jQuery(window).width(); if (action === 'active') {
        var li = element.closest('li'); if (li.hasClass('current') || li.hasClass('entered')) { return false; }
        li.closest('ul').children().removeClass('entered'); li.addClass('entered'); var elWidth = li.outerWidth(); var textWidth = li.find('span').outerWidth(); li.css({ width: elWidth + textWidth + 'px' });
    } else {
        var li = element.closest('li'); var allLi = li.closest('ul').children(); var fixedLi = li.closest('ul').children('.fixed'); var activeLi = li.closest('ul').children('.current'); allLi.removeClass('entered').css({ width: '70px' }); if (ww <= 1400) { allLi.removeClass('entered').css({ width: '50px' }); }
        if (activeLi.length) {
            var elWidth = 70; if (ww <= 1400) { elWidth = 50; }
...
function iknow_tm_menu_width_new() {
    "use strict"; var ww = jQuery(window).width(); var btn = jQuery('.iknow_tm_mainpart_new_2 .main_menu ul li a'); btn.on('mouseenter', function () { iknow_tm_menu_width_action(jQuery(this), 'active'); }).on('mouseleave', function () { iknow_tm_menu_width_action(jQuery(this), ''); }); btn.on('click', function () {
        var element = jQuery(this); var li = element.closest('li'); if (li.hasClass('current') && !li.hasClass('entered')) { return false; }
        ...
        return false;
    }); jQuery('.iknow_tm_mainpart_new_2 .main_menu ul li.current').each(function () { var element = jQuery(this); var elWidth = element.outerWidth(); var textWidth = element.find('span').outerWidth(); element.css({ width: elWidth + textWidth + 'px' }); });
}
